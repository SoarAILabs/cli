# Kite CLI Execution and Deployment

This document describes how to run Kite CLI and explains the deployment architecture that Kite CLI uses.

## Running Kite CLI

There are several ways to run Kite CLI. The option you choose depends on how you intend to use Kite CLI.

---

### 1. Standard installation (Recommended for typical users)

This is the recommended way for end-users to install Kite CLI. It involves downloading the Kite CLI package from the NPM registry.
...
For security and isolation, Kite CLI can be run inside a container. This is the default way that the CLI executes tools that might have side effects.
...
- **Using the `--sandbox` flag:**
  If you have Kite CLI installed locally (using the standard installation described above), you can instruct it to run inside the sandbox container.
  ```bash
  kite --sandbox -y -p "your prompt here"
  ```

---

### 3. Running from source (Recommended for Kite CLI contributors)
